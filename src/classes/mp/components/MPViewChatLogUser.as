package classes.mp.components
{
    ...
    public class MPViewChatLogUser extends Sprite
    {
        ...
        public function MPViewChatLogUser(parent:DisplayObjectContainer = null, xpos:Number = 0, ypos:Number = 0, wid:Number = 0, hei:Number = 0):void
        {
            this.x = xpos;
            this.y = ypos;
        ...
        public function update(e:MPUserEvent):void
        {
            ...
            var i:int;
            for (i = history.messages.length - 1; i >= 0; i--)
            {
            ...
            for (i = found ? i + 1 : 0; i < history.messages.length; i++)
        ...
        private function loadHistory():void
        {
            ...
            _cachePositions.length = 0;

            var startY:int = 5;
            var endY:int;

            for (var i:int = 0; i < history.messages.length; i++)
            {
                const entry:MPChatLogEntry = history.messages[i];

                entry.build(pane.width - 1);

                endY = startY + entry.height;
